              ...
              return (
                <div key={message.id} className="space-y-2 message-appear">
                  ...
                  <div className="flex justify-end">
                    <div className="bg-blue-100 px-3 py-2 rounded-lg max-w-[85%] shadow-sm message-bubble text-sm">
                      <p className="break-words leading-relaxed text-blue-900 font-medium">{message.userQuery}</p>
                    </div>
                  </div>
      ...
      {(enableTextSelection || onTextSelect !== undefined) && showInsertButton && selectedText && (
        ...
        >
          <button
            ...
            title={t('response.insert.tooltip')}
          >
            <svg 
            ...
            <span className="font-semibold">{t('response.insert')}</span>
          </button>
        </div>
        )}

      {/* QueryForm intégré dans la colonne de chat - fixé en bas */}
      {onQuerySubmit && (
        <div className="border-t border-gray-200 bg-white flex-shrink-0">
          <QueryForm
            isFileContentVisible={isFileContentVisible || false}
