...
interface SidebarProps {
  ...
  getRepoStructure: (structureFn: () => string, files: File[]) => void; // Updated to pass files
...
const Sidebar: React.FC<SidebarProps> = ({ onFileSelect, getRepoStructure, selectedModel, setSelectedModel }) => {
  ...
  // New function to extract all File objects from the FileNode tree
  const extractAllFiles = (nodes: FileNode[]): File[] => {
    const allFiles: File[] = [];
    nodes.forEach((node) => {
      if (node.file) {
  ...
  useEffect(() => {
    ...
    const allFiles = extractAllFiles(files); // Extract all File objects
    getRepoStructure(() => structure, allFiles); // Pass both structure and files
