export const frFR = {
  // Language Switcher
  'lang.english': 'English',
  'lang.french': 'Français',
  'lang.spanish': 'Español',

  // ChatPanel
  'chat.title': 'Chat',
  'chat.empty.title': 'Que souhaitez-vous savoir ?',
  'chat.empty.description': 'Commencez par poser une question ou télécharger un fichier pour obtenir des informations.',

  // QueryForm
  'query.placeholder': 'Que souhaitez-vous savoir ?',
  'query.autoModel': 'Sélection automatique du modèle',
  'query.research': 'Recherche',
  'query.local': 'Local',
  'query.file': 'Fichier',
  'query.send': 'Envoyer',
  'query.mobile.help': 'Appuyez sur Entrée pour envoyer, Maj+Entrée pour une nouvelle ligne',

  // FileViewer
  'fileviewer.dragDrop': 'Glissez-déposez un fichier ou un répertoire',
  'fileviewer.selectFile': 'Sélectionner un fichier',
  'fileviewer.uploadHint': 'Téléchargez des documents pour commencer à les analyser avec l\'IA',
  'fileviewer.directory.selected': 'Répertoire sélectionné',
  'fileviewer.directory.selectFile': 'Sélectionnez un fichier du répertoire dans la barre latérale pour commencer l\'analyse',
  'fileviewer.directory.filesAvailable': 'fichiers disponibles',
  'fileviewer.zoom.out': 'Zoom arrière',
  'fileviewer.zoom.in': 'Zoom avant',
  'fileviewer.page.previous': 'Page précédente',
  'fileviewer.page.next': 'Page suivante',
  'fileviewer.page.search': 'Rechercher',
  'fileviewer.page.of': 'sur',
  'fileviewer.search.prompt': 'Rechercher dans le document',
  'fileviewer.processing': 'Traitement des fichiers...',
  'fileviewer.pdf.loading': 'Chargement du PDF...',
  'fileviewer.pdf.error': 'Erreur lors du chargement du PDF',
  'fileviewer.docx.error': 'Erreur lors du chargement du DOCX',
  'fileviewer.unsupported': 'Type de fichier non supporté',

  // ResponseDisplay
  'response.insert': 'Insérer',
  'response.insert.tooltip': 'Insérer ce texte dans le document',

  // Sidebar - Panels & Tabs
  'sidebar.noFiles': 'Aucun fichier',
  'sidebar.file': 'Fichier',
  'sidebar.export': 'Exporter',
  'sidebar.model': 'Modèle',
  'sidebar.upload': 'Télécharger',
  'sidebar.connector': 'Connecteur',
  'sidebar.agents': 'Agents',
  'sidebar.organisation': 'Organisation',
  'sidebar.history': 'Historique',
  'sidebar.settings': 'Paramètres',
  'sidebar.account': 'Compte',
  'sidebar.recent.searches': 'Recherches récentes',
  'sidebar.noHistory': 'Aucun historique pour le moment',
  'sidebar.openFiles': 'Fichiers ouverts',
  'sidebar.recentDocuments': 'Documents récents',

  // Settings Tab
  'settings.theme': 'Thème',
  'settings.language': 'Langue',
  'settings.themeWhite': 'Blanc',
  'settings.themeDark': 'Sombre',
  'settings.themeDarkBlue': 'Bleu foncé',

  // Account Tab
  'account.title': 'Compte',
  'account.plan': 'Plan',
  'account.signOut': 'Se déconnecter',
  'account.notSignedIn': 'Non connecté',
  'account.signIn': 'Se connecter',

  // Organisation Tabs
  'org.tab.organisation': 'Organisation',
  'org.tab.user': 'Utilisateur',
  'org.tab.permission': 'Permission',
  'org.activeOrganisation': 'Organisation active',
  'org.noOrganisations': '— Aucune organisation —',
  'org.manage': 'Organisations',
  'org.manageDepartments': 'Départements',
  'org.manageUsers': 'Créer un utilisateur',
  'org.noUsers': 'Aucun utilisateur pour le moment',
  'org.createUser': 'Créer un utilisateur',
  'org.selectRole': '— Sélectionner un rôle —',
  'org.email': 'E-mail',
  'org.password': 'Mot de passe',
  'org.fullName': 'Nom complet',
  'org.departmentName': 'Nom du département',
  'org.organisationName': 'Nom de l\'organisation',
  'org.noDepartments': 'Aucun département pour le moment',
  'org.created': 'Organisation créée !',
  'org.updated': 'Organisation mise à jour !',
  'org.deleted': 'Organisation supprimée !',
  'org.deleteError': 'Échec de la suppression',
  'org.deptUpdated': 'Département mis à jour !',
  'org.deptDeleted': 'Département supprimé !',
  'org.deptCreated': 'Département créé !',
  'org.userCreated': 'Utilisateur créé !',

  // Organisation Management
  'org.title': 'Gestion des organisations',
  'org.organisations': 'Organisations',
  'org.departments': 'Départements',
  'org.users': 'Utilisateurs',
  'org.roles': 'Rôles',
  'org.permissions': 'Permissions',

  // Roles
  'role.title': 'Gérer les rôles',
  'role.name': 'Nom',
  'role.organisation': 'Organisation',
  'role.created': 'Rôle créé !',
  'role.updated': 'Rôle mis à jour !',
  'role.deleted': 'Rôle supprimé !',

  // Permissions
  'permission.title': 'Gérer les permissions',
  'permission.saved': 'Permissions sauvegardées',
  'permission.add': 'Ajouter une permission',
  'permission.role': 'Rôle',
  'permission.roleLabel': 'Rôles',
  'permission.resource': 'Ressource',
  'permission.resourceLabel': 'Ressources',
  'permission.action': 'Actions',
  'permission.actionLabel': 'Actions',
  'permission.validFrom': 'Valide à partir de',
  'permission.validFromLabel': 'Valide à partir de',
  'permission.validTo': 'Valide jusqu\'au',
  'permission.validToLabel': 'Valide jusqu\'au',
  'permission.selectRole': '— Sélectionner un rôle —',
  'permission.selectResource': '— Sélectionner une ressource —',
  'permission.selectActions': '— Sélectionner des actions —',
  'permission.added': 'Permission ajoutée !',
  'permission.deleted': 'Permission supprimée !',
  'permission.deletedPlural': 'Permissions supprimées !',
  'permission.notFound': 'Permission non trouvée',
  'permission.errorDeleting': 'Erreur lors de la suppression de la permission',

  // Permissions - Success Messages
  'permission.created.count': '✓ {count} permission{plural} ajoutée{plural} !',
  'permission.duplicate.count': '⚠ {count} existent déjà.',
  'permission.duplicate.all': '⚠ Toutes les permissions existent déjà pour ce rôle.',

  // Common
  'common.pages': 'pages',
  'common.page': 'page',
  'common.file': 'fichier',
  'common.select': 'Sélectionner',
  'common.save': 'Enregistrer',
  'common.delete': 'Supprimer',
  'common.cancel': 'Annuler',
  'common.create': 'Créer',
  'common.edit': 'Éditer',
  'common.close': 'Fermer',

  // Status messages
  'status.extracting.pdf': 'Extraction du texte du PDF {fileName}...',
  'status.extracting.docx': 'Extraction du texte du document {fileName}...',
  'status.reading.file': 'Lecture du fichier {fileName}...',
  'status.analyzing.document': 'Analyse du document {fileName}...',
  'status.sending.server': 'Envoi au serveur pour analyse...',
  'status.ai.analyzing': 'Analyse en cours par l\'IA...',
  'status.generating.summary': 'Génération du résumé...',
  'status.extracting.content': 'Extraction du contenu de {count} fichier{plural}...',
  'status.analyzing.repository': 'Analyse du répertoire ({count} fichier{plural})...',
  'status.generating.repo.summary': 'Génération du résumé du répertoire...',
  'status.processing.file': 'Traitement de {fileName}...',
  'status.processing.files': 'Traitement de {count} fichiers...',
  'status.extracting.files': 'Extraction des fichiers...',
  'status.analyzing.question': 'Analyse de votre question...',
  'status.processing.request': 'Traitement de votre requête...',
  'status.analyzing.file': 'Analyse du fichier {fileName}...',
  'status.searching.documents': 'Recherche dans les documents...',
  'status.searching.online': 'Recherche d\'informations en ligne...',
  'status.generating.response': 'Génération de la réponse...',
  'status.extracting.structured': 'Extraction des données structurées en cours...',
  'status.indexing.extracting': 'Extraction du contenu de {count} fichiers...',
  'status.indexing.processing': 'Traitement de {fileName} ({current}/{total})...',
  'status.indexing.on.server': 'Indexation sur le serveur ({count} fichiers)...',
  'status.indexing.documents': 'Indexation des documents en cours...',

  // Error messages
  'error.page.limit.exceeded': '⚠️ Le fichier "{fileName}" a {pageCount} pages. La limite est de 100 pages par document.',
  'error.page.limit.exceeded.ignored': '⚠️ Le fichier "{fileName}" a {pageCount} pages. La limite est de 100 pages par document. Ce fichier sera ignoré.',
  'error.page.limit.exceeded.generic': '⚠️ Le fichier "{fileName}" dépasse la limite de 100 pages.',
  'error.page.limit.exceeded.query': '⚠️ Le fichier "{fileName}" a {pageCount} pages. La limite est de 100 pages par document. Veuillez sélectionner un autre fichier pour poser des questions.',
  'error.page.limit.exceeded.query.generic': '⚠️ Le fichier "{fileName}" dépasse la limite de 100 pages. Veuillez sélectionner un autre fichier pour poser des questions.',

  // Reasoning animation
  'reasoning.analyzing.question': '🧠 Analyse de votre question...',
  'reasoning.analyzing.description': 'Compréhension du contexte et des exigences',
  'reasoning.gathering.info': '🔍 Collecte d\'informations pertinentes...',
  'reasoning.gathering.description': 'Recherche dans le contenu et la structure des fichiers',
  'reasoning.processing': '⚡ Traitement avec raisonnement avancé...',
  'reasoning.processing.description': 'GPT-5 réfléchit profondément à votre demande',
  'reasoning.formulating': '🎯 Formulation d\'une réponse complète...',
  'reasoning.formulating.description': 'Création d\'une réponse détaillée et précise',
  'reasoning.finalizing': '✨ Finalisation de la réponse...',
  'reasoning.finalizing.description': 'Ajout des dernières touches et mise en forme',
  'reasoning.step': 'Étape {current} sur {total}',
  'reasoning.powered.by': 'Alimenté par',

  // Error messages
  'app.errors.dailyQueries': 'Erreur lors de la lecture des requêtes quotidiennes',

  // App Home Component
  'app.feature1': 'peut rechercher dans toute votre organisation',
  'app.feature2': 'peut aider chaque employé à travailler comme un expert',
  'app.feature3': 'peut analyser vos bases de données en anglais simple',
  'app.feature4': 'peut détecter les anomalies avant qu\'elles ne deviennent des problèmes',
  'app.logoAlt': 'Logo Flexi',
  'app.close': 'Fermer',
  'app.closeAlert': 'Fermer l\'alerte',
  'query.limitReached': 'Vous avez atteint la limite de 5 requêtes par jour. Veuillez vous connecter pour continuer à utiliser FlexiAnalyse.',
  'query.inputLabel': 'Entrez votre question ou requête',
  'query.selectModel': 'Sélectionner un modèle',
  'query.toggleMode': 'Basculer le mode de recherche',
  'query.submit': 'Envoyer la requête',

  // Connector translations
  'connector.newConnection': 'Nouvelle connexion',
  'connector.newConnectionName': 'Nouveau {service}',
  'connector.fields.name': 'Nom de la connexion',
  'connector.fields.folderId': 'ID du dossier racine (optionnel)',
  'connector.fields.tenantId': 'ID du locataire',
  'connector.fields.siteUrl': 'URL du site',
  'connector.fields.connectionUrl': 'URL de connexion',
  'connector.placeholders.myGoogleDrive': 'Mon Google Drive',
  'connector.placeholders.folderId': '1BxiMVs0XRA5…',
  'connector.placeholders.myDropbox': 'Mon Dropbox',
  'connector.placeholders.mySharePoint': 'Mon SharePoint',
  'connector.placeholders.tenantId': 'xxxxxxxx-xxxx-…',
  'connector.placeholders.siteUrl': 'https://company.sharepoint.com/sites/…',
  'connector.placeholders.myDatabase': 'Ma base de données',
  'connector.placeholders.connectionUrl': 'postgresql://user:pass@host:5432/db',
  'connector.oauthSecured': 'Sécurisé via {service} OAuth',
  'connector.sharepointDetails': 'et les détails du locataire',
  'connector.thenClick': 'Remplissez le nom',
  'connector.connect': 'Connecter',
  'connector.cancel': 'Annuler',
  'connector.connecting': 'Connexion…',
  'connector.connectService': 'Connecter {service}',
  'connector.saveConnection': 'Enregistrer la connexion',
  'connector.savedOpening': 'Enregistré — ouverture de l\'autorisation {service}…',
  'connector.addedSuccessfully': 'Connecteur ajouté avec succès !',
  'connector.error': 'Erreur : {message}',
  'connector.dbLimit': 'Votre plan permet {count} base{plural} de données. Passez à un plan supérieur pour en connecter davantage.',
  'connector.errors.connectionUrlRequired': 'L\'URL de connexion est requise',
  'connector.errors.tenantIdRequired': 'L\'ID du locataire est requis',
  'connector.errors.siteUrlRequired': 'L\'URL du site est requise',

  // Model translations
  'model.selectModel': 'Sélectionner un modèle',
  'model.auto': 'Auto (recommandé)',
  'model.gpt-3.5-turbo': 'GPT-3.5 Turbo',
  'model.gpt-4o': 'GPT-4o',
  'model.gpt-5': 'GPT-5',
  'model.mistral': 'Mistral Medium',
  'model.llama3': 'Llama 3.2',
  'model.latest': 'Dernier',
  'model.provider.smartSelector': 'Sélecteur intelligent',
  'model.provider.OpenAI': 'OpenAI',
  'model.provider.MistralAI': 'Mistral AI',
  'model.provider.Local': 'Local',

  // DbChatPanel (agent base de données)
  'dbchat.liveSql': 'SQL exécuté',
  'dbchat.write.title': 'Écriture — confirmation requise',
  'dbchat.write.impactUnknown': 'Impact inconnu.',
  'dbchat.write.willModify': 'Cette opération modifiera',
  'dbchat.write.rowsBold': '{count} ligne{plural}',
  'dbchat.write.bigImpact': '⚠️ Impact important (> 1000 lignes) — vérifie bien avant de confirmer.',
  'dbchat.write.executing': 'Exécution…',
  'dbchat.write.confirm': 'Confirmer & exécuter',
  'dbchat.write.cancelled': 'Annulé — aucune modification.',
  'dbchat.write.failed': 'Échec.',
  'dbchat.history.tooltip': 'Historique des conversations',
  'dbchat.history.empty': 'Aucune conversation passée.',
  'dbchat.newSearch.tooltip': 'Démarrer une nouvelle recherche',
  'dbchat.newSearch.label': 'Nouvelle recherche',
  'dbchat.untitled': 'Sans titre',
  'dbchat.followUp.placeholder': 'Poser une question de suivi…',

  // ScopeSelector
  'scope.perimeter': 'Périmètre de recherche',
  'scope.allContext': 'Tout le contexte',

  // SuggestionChips
  'suggest.analysing': 'Analyse de votre base de données…',
  'suggest.diagramTooltip': 'Afficher le diagramme du schéma de la base',
  'suggest.diagram': 'Diagramme de la base',
  'suggest.reportTooltip': 'Afficher le rapport de santé de la base',
  'suggest.report': 'Rapport de la base',

  // SearchProgress
  'progress.understand': 'Compréhension de votre question',
  'progress.schema': 'Lecture du schéma de la base',
  'progress.sql': 'Génération de la requête SQL',
  'progress.run': 'Exécution de la requête',
  'progress.compose': 'Composition de la réponse',

  // DbResultGrid
  'grid.result': 'Résultat',
  'grid.rowsOf': '{shown} sur {total} lignes',
  'grid.rows': '{count} ligne{plural}',
  'grid.cols': '{count} col{plural}',
  'grid.empty': 'Posez une question sur vos données et le tableau de résultats apparaît ici.',
  'grid.page': 'Page {current} / {total}',
  'grid.first': 'Première page',
  'grid.last': 'Dernière page',

  // SchemaFlow (diagramme ER)
  'schema.junctionTooltip': 'Table de jonction (relation N:N)',
  'schema.rowsApprox': '~{count} lignes',
  'schema.search': 'Rechercher une table…',
  'schema.reset': 'Réinitialiser',
  'schema.legend.fk': 'relation clé étrangère (un → plusieurs)',
  'schema.legend.junction': 'table de jonction (plusieurs ↔ plusieurs)',
  'schema.tip.junction': '{source} → {target} : {target} est une table de jonction (relation N:N)',
  'schema.tip.oneToMany': '{source} → {target} : plusieurs {source} pour un {target} (1:N)',

  // AppHomeComponent
  'home.diagram.title': 'Schéma de la base',
  'home.diagram.hideAuditTooltip': 'Masque les tables d\'audit / log / système du diagramme et les exclut des réponses de l\'agent',
  'home.diagram.hideAudit': 'Masquer les tables d\'audit',
  'home.diagram.backToResults': '← Retour aux résultats',
  'home.diagram.noSchema': 'Aucun schéma disponible',
  'home.detail.table': 'Table',
  'home.detail.loading': 'Analyse…',
  'home.detail.columns': 'Colonnes',
  'home.detail.rows': 'Lignes',
  'home.detail.columnsNulls': 'Colonnes · nuls / non-nuls',
  'home.detail.statsSkipped': 'Stats nuls non calculées (table volumineuse).',
  'home.detail.nonNull': '{count} non-nuls',
  'home.detail.null': '{count} nuls',
  'home.error.hostRequired': 'L\'hôte est requis',
  'home.error.serviceNameRequired': 'Le nom du service est requis',
  'home.error.databaseRequired': 'La base de données est requise',
  'home.write.refused': 'Écriture refusée.',
  'home.write.rowsModified': '{count} ligne{plural} modifiée{plural}.',

  // PlansView
  'plans.title': 'Plans & tarifs',
  'plans.subtitle': 'Choisissez le palier adapté à votre usage.',
  'plans.loading': 'Chargement…',
  'plans.current': 'Actuel',
  'plans.currentPlan': 'Plan actuel',
  'plans.onQuote': 'Sur devis',
  'plans.free': 'Gratuit',
  'plans.paymentSoon': 'Le paiement en ligne arrive bientôt — pour changer de plan, contactez-nous.',
  'plans.emailSubject': 'FlexiAnalyse — Passer au plan {plan}',

  // Sidebar — panneaux connecteur / dial-home / agents
  'sb.msg.authorized': 'Connecteur autorisé — ingestion démarrée !',
  'sb.msg.authFailed': 'Autorisation échouée : {reason}',
  'sb.msg.ingestionStarted': 'Ingestion démarrée !',
  'sb.msg.resyncStarted': 'Re-synchronisation du schéma démarrée !',
  'sb.msg.connected': 'Connecté ({dialect}) — {count} table{plural} détectée{plural}.',
  'sb.msg.connectionFailed': 'Connexion échouée.',
  'sb.msg.localCreated': 'Connecteur local créé — lance la commande de l\'agent.',
  'sb.msg.updated': 'Connexion mise à jour !',
  'sb.msg.opening': 'Ouverture de l\'autorisation {provider}…',
  'sb.error.selectOrg': 'Sélectionne d\'abord une organisation (onglet Organisation)',
  'sb.error.tenantSiteRequired': 'L\'ID du locataire et l\'URL du site sont requis',
  'sb.oauth.via': 'L\'autorisation se fait via {provider}.',
  'sb.oauth.fillPrefix': 'Renseigne le nom{details}, puis clique sur ',
  'sb.oauth.tenantDetails': ' et les détails du locataire',
  'sb.mode.cloud': 'Base accessible',
  'sb.mode.local': 'Base locale / on-prem',
  'sb.local.name': 'Nom',
  'sb.local.namePlaceholder': 'Ma base locale',
  'sb.local.explainPrefix': 'Ta base reste chez toi. On génère une commande Docker à lancer sur une machine de ton réseau ; l\'agent se connecte à nous en ',
  'sb.local.explainStrong': 'sortant',
  'sb.local.explainSuffix': ' (aucun port à ouvrir). Les identifiants ne quittent jamais ta machine.',
  'sb.local.agentOnline': 'Agent en ligne',
  'sb.local.agentWaiting': 'En attente de l\'agent…',
  'sb.local.replacePrefix': 'Remplace ',
  'sb.local.replaceSuffix': ' par ta base, puis lance sur une machine du réseau :',
  'sb.local.copyCommand': 'Copier la commande',
  'sb.local.connected': 'Connecté — ferme, puis synchronise le schéma.',
  'sb.connectors.title': 'Connecteurs de base de données',
  'sb.connectors.saved': 'Connexions enregistrées',
  'sb.connectors.empty': 'Aucune connexion enregistrée pour le moment.',
  'sb.refresh': 'Actualiser',
  'sb.action.sync': 'Synchroniser / ingérer',
  'sb.agents.empty': 'Aucun agent disponible pour le moment.',
  'sb.agents.hint': 'Les agents IA apparaîtront ici au fur et à mesure.',
  'sb.conversations': 'Conversations',
};
