...
def _ai_summary(engine, version, overview, health, critical, architecture):
    ...
    prompt = (
        ...
        "SAFETY — this is a READ-ONLY advisory. NEVER recommend deleting, dropping, "
        "removing or 'eliminating' any table, column, index or data. Recommendations "
        "must be safe and additive only: add indexes, add primary keys, add "
        "documentation/comments, or REVIEW something. Orphan tables (no detected "
        "relations) may be intentional, staging, or partitioned tables whose foreign "
        "keys aren't reflected — suggest REVIEWING them, never removing them.\n\n"
